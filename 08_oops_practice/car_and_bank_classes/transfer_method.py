class BankAccount:
    ...
    def transfer(self, target_account, amount):
        if amount > 0:
            if amount <= self.balance:
                if self.balance - amount < BankAccount.min_balance():
                    print(f"Cannot transfer ${amount:.2f}. Minimum balance of ${BankAccount.min_balance():.2f} must be maintained.")
                    return
                self.balance -= amount
...
if len(accounts) >= 2:
    ...
    for i, account in enumerate(accounts):
        print(f"{i + 1}. {account.account_holder} (Balance: ${account.get_balance():.2f})")
    from_index = int(input("Select the account number to transfer from: ")) - 1
    to_index = int(input("Select the account number to transfer to: ")) - 1
    transfer_amount = float(input("Enter amount to transfer: "))
    accounts[from_index].transfer(accounts[to_index], transfer_amount)
